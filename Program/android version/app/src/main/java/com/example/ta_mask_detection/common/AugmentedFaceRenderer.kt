...
import com.example.ta_mask_detection.common.rendering.ShaderUtil.loadGLShader
...
class AugmentedFaceRenderer {
    ...
    private var colorCorrectionParametersUniform = 0
    ...
    private val lightingDirection = floatArrayOf(0.0f, 1.0f, 0.0f, 0.0f)
    private var program = 0
    private val modelViewProjectionMat = FloatArray(16)
    private val modelViewMat = FloatArray(16)
    private val viewLighDirection = FloatArray(4)

    fun AugmentedFaceRenderer() {}

    @Throws(IOException::class)
    fun createOnGlThread(context: Context, diffuseTextureAssetName: String){
        val vertexShader: Int = loadGLShader(TAG, context, GLES20.GL_VERTEX_SHADER, Companion.VERTEX_SHADER_NAME)
        val fragmentShader: Int = loadGLShader(TAG, context, GLES20.GL_FRAGMENT_SHADER, Companion.FRAGMENT_SHADER_NAME )
    ...
    fun draw(projmtx: FloatArray?, viewmtx: FloatArray?, modelmtx: FloatArray?, colorCorrectionRgba: FloatArray?, face: AugmentedFace){
        ...
        Matrix.multiplyMV(viewLighDirection, 0, modelViewMat, 0, lightingDirection, 0)
        normalizeVec3(viewLighDirection)
        GLES20.glUniform4f(lightingParametersUniform, viewLighDirection[0], viewLighDirection[1], viewLighDirection[2], 1f)
        ...
        GLES20.glDrawElements(GLES20.GL_TRIANGLES, triangleIndices.limit(), GLES20.GL_UNSIGNED_SHORT, triangleIndices)
